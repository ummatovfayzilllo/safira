...
export class PurcachedCoursesService {
  ...
  async create(data: CreatePurcachedCourseDto) {
    ...
    if (data.amount !== course.price)
      throw new BadRequestException(
        `Your amount [${data.amount}] noequel in course price [${course.price}]`,
      );
    if (!course.published)
      throw new BadRequestException(`[${course.name}] not published `);
    ...
    const oldPurcached = await this.prisma.purcachedCourse.findMany(
      purcachedCourseFindEntity(data),
    );

    if (oldPurcached[0])
      throw new ConflictException(
        `PurcachedCourse by userId [${data.userId}] and by courseId [${data.courseId}] already exists !`,
      );
  ...
  async findOne(id: string) {
    return {
      ...
      data: await checkExistsResurs(
        this.prisma,
        ModelsEnumInPrisma.PURCACHED_COURSES,
        'id',
        id,
      ),
    };
  ...
  async update(id: string, data: UpdatePurcachedCourseDto) {
    ...
    try {
      return {
        ...
        data: await this.prisma.purcachedCourse.update({
          where: { id: id },
          data: { ...data },
        }),
      };
