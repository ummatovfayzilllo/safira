...
export class HomeworkSubmissionsService {
  ...
  async create(
    ...
    const oldHomeworkSubmission =
      await this.prisma.homeworkSubmission.findFirst({
        where: {
          AND: [{ userId: data.userId }, { homeworkId: data.homeworkId }],
        },
      });
    if (files) {
    ...
    if (oldHomeworkSubmission) {
      try {
        return {
          message:
            'Homework submission already exists and updated homework submission !',
  ...
  async update(
    ...
    const oldHomeworkSubmission = await checkExistsResurs<HomeworkSubmission>(
      this.prisma,
      ModelsEnumInPrisma.HOMEWORK_SUBMISSIONS,
      'id',
      id,
  ...
  async remove(id: string) {
    ...
    try {
      const deletedData = await this.prisma.homeworkSubmission.delete({
        where: { id: id },
      });
      if (oldHomeworkSubmission.files) {
        oldHomeworkSubmission.files.map(async (file) => {
